...
public class BoardTestSuite {
    ...
    void testAddTaskListFindOutdatedTasks() {
        ...
        List<Task> tasks = project.getTaskLists().stream()          // [4]
                .filter(task -> undoneTasks.contains(task))                           // [5]
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        //When
        List<TaskList> checkedTasks = new ArrayList<>();
        checkedTasks.add(new TaskList("In progress"));
        double averageTime = project.getTaskLists().stream()
                .filter(checkedTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .map(date -> ChronoUnit.DAYS.between(date, LocalDate.now()))
                .mapToInt(dayNumber -> dayNumber.intValue())
                        .average().orElse(0.0);

        //Then
        assertEquals(10.0, averageTime, 0.01);
    }
